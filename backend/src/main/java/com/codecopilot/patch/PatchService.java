...
public class PatchService {
	...
	public PatchService(GeneratedPatchRepository patchRepository, SearchService searchService,
			...
			GitRepositoryRepository repositoryRepository, AiService aiService, ProjectAccessService accessService,
	...
	public GeneratedPatch generate(Long projectId, PatchRequest request) {
		...
		auditService.log(projectId, "PATCH_GENERATED", "generated_patch:" + patch.getId(), request.getInstruction());
		return patch;
	}
	...
	private String extractSummary(String diff) {
		...
		return java.util.Arrays.stream(lines)
				.filter(l -> l.startsWith("+") && !l.startsWith("+++") && !l.startsWith("+-")).limit(6)
