...
public class DocumentationService {
    ...
    private String buildCorpus(Long projectId, Long repositoryId, Project project) {
        ...
        for (CodeClass c : classes) {
            sb.append("- ").append(c.getFqName()).append(" (")
                    .append(c.getKind()).append(") ").append(describe(stereotype(c))).append("\n");
        }
        sb.append("\nAPI endpoints:\n");
        List<CodeMethod> endpoints = methodRepository.findByProjectIdAndRepositoryIdAndHttpPathNotNull(projectId, repositoryId);
        for (CodeMethod m : endpoints) {
            sb.append("- ").append(m.getHttpMethod()).append(' ').append(m.getHttpPath())
                    .append(" -> ").append(canonicalClass(classes, m)).append('.').append(m.getName()).append("\n");
        }
        sb.append("\nDependencies:\n");
        ...
        for (CodeDependency d : deps) {
            sb.append("- ").append(d.getSourceClassFq()).append(" --")
                    .append(d.getType()).append("--> ").append(d.getTargetClassFq()).append("\n");
        }
        return sb.toString();
    }

    private String canonicalClass(List<CodeClass> classes, CodeMethod m) {
        return classes.stream().filter(c -> c.getId().equals(m.getClassId()))
                .map(CodeClass::getName).findFirst().orElse("?");
    }
    ...
    private String stereotype(CodeClass c) {
        ...
        if (anns.contains("Service")) return "service";
        if (anns.contains("Repository")) return "repository";
        if (anns.contains("Entity")) return "entity";
        if (anns.contains("Configuration")) return "configuration";
        if ("INTERFACE".equals(c.getKind())) return "interface";
        return "model";
    }
