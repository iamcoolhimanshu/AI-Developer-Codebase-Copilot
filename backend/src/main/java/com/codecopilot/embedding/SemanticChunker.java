...
/**
 * Turns parsed source files into focused, embeddable chunks. Java files are
 * chunked per class + per method (semantically meaningful units); other text
 * files are split into fixed-size windows.
 */
@Component
public class SemanticChunker {
    ...
    public List<ChunkSpec> chunkJava(ParsedSourceFile parsed) {
        ...
        for (ParsedSourceFile.ParsedType type : parsed.types()) {
            // Class-level overview chunk (header, annotations, fields)
            ...
            for (ParsedSourceFile.ParsedMethod method : type.methods()) {
                ...
                out.add(new ChunkSpec("METHOD", type.name(), method.name(),
                        method.startLine(), method.endLine(), content));
            }
        }
        return out;
    }
